...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.27**（贴在最前只置顶当前窗；保存配置记住文件夹后自动写）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 浮窗：点「对局浮窗」用独立窗口打开同一份 `王者助手.html#hud-阵容码`（关掉原助手页小窗还在）。原页切到「浮窗已打开」占位。小窗「回到助手」只唤醒/新开完整大窗口，不把主页塞进小窗。「贴在最前」只聚焦当前小窗，不再另开画中画窗口。用户可见名是「助手」不是「图鉴」。关键按钮加重显示。弹窗 × 直接关层，不依赖 `history.back()`（file:// 常失效）。
- 尺寸：默认弹出系统小窗（可拉到接近整屏高），站位在上、装备/运营/出牌在下，不再左右分栏。画中画「贴在最前」仍可选用。格子大小随屏幕高度。贴在屏幕右侧。手拉尺寸按分辨率分桶记 `wxq-hud-size`。顶栏「适配屏幕」清掉本分辨率手拉记录并重算。
...
- 各电脑直接打开该 HTML，会自动读同目录 `王者助手.json.js`，**不用导入**。第一次点「保存配置」选一次「王者万象棋助手」文件夹，目录句柄存 IndexedDB，之后改收藏/尺寸/主题会自动写回，不再弹选文件夹。授权丢了只提示一次，再点保存配置即可恢复。发布脚本从坚果云 `UsersMap.json` 反查根目录（`NUTSTORE_ROOT` 仍可覆盖）。
- 浮窗：复制失败弹出大字阵容码（点一下全选再 Ctrl+C）。顶栏只留「适配屏幕/图鉴开/贴在最前/弹出小窗/关闭」。手机（触屏/窄屏）走底部抽屉：站位置顶吸住，下滑看装备/运营/出牌。
