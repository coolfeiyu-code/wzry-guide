...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.15**（浮窗按屏幕给尺寸，可自由拉伸）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 尺寸：按屏幕 `availWidth/Height` 给初始宽高（大约宽 26–34%、高 80%），记住 `wxq-hud-size`。页内面板 `resize:both` 无 max 限制，弹出窗 `resizable=yes`。拉到 ≥560px 宽时站位在左、装备/运营/出牌在右。
