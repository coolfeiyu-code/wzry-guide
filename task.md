...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.30**（改用静默自动保存与打开即读）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 各电脑直接打开该 HTML：发布版开头就是 `<script src="王者助手.json.js">`，页面脚本跑之前配置已生效，**不用导入、不用点**。收藏/取消收藏、拖动浮窗、改主题会在 250ms 后静默写回，全程无弹窗。浏览器规定「写文件夹」必须先授权一次，所以第一次要点一下左下角「开启自动保存」并选一次「王者万象棋助手」文件夹；之后这台机器一直静默，按钮自己隐藏。发布脚本从坚果云 `UsersMap.json` 反查根目录（`NUTSTORE_ROOT` 仍可覆盖）。
