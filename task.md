...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.25**（小窗吸壁缩进）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 浮窗：点「对局浮窗」用独立窗口打开同一份 `王者助手.html#hud-阵容码`（关掉原助手页小窗还在）。原页切到「浮窗已打开」占位。小窗顶栏大红「回到助手」切回完整页。用户可见名是「助手」不是「图鉴」。关键按钮（对局浮窗、保存配置、右下角码头）加重显示。
- 吸壁：小窗顶栏左/右/上/下把窗口吸到该侧。鼠标移开约 0.7 秒缩进只留一条边，移回去展开。也可把窗口拖近某条边再移开鼠标。
...
- 各电脑直接打开该 HTML，会自动读同目录 `王者助手.json.js`，**不用导入**。点「保存配置」才写入/覆盖这份文件（后保存的覆盖先保存的）。不点保存，只留在这台浏览器。第一次保存要选坚果云里的「王者万象棋助手」文件夹。发布脚本从坚果云 `UsersMap.json` 反查根目录（`NUTSTORE_ROOT` 仍可覆盖）。
