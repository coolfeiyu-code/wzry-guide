...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.34**（对局浮窗恢复为真正独立小窗）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- **为什么必须用 replaceState 而不是 pushState**：Chrome 只在窗口历史仍是 1 条时才允许脚本关闭自己开的窗口。实测 `history.pushState` 过就关不掉（窗口留原地）。所以浮窗内切套用 `location.hash`/`replaceState`，**绝不 pushState**。
- 兜底：若助手页关不掉（例如动过历史），就停在占位页，内容不显示，也不再叠一个浮窗内容。开窗被浏览器拦截时，就地切成浮窗视图。
