...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.29**（保存配置秒写：授权后不再查 IndexedDB）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 各电脑直接打开该 HTML，会自动读同目录 `王者助手.json.js`，**不用导入**。句柄在启动时从 IndexedDB 取回内存并确认授权；已授权时点「保存配置」直接写文件，不再查 IndexedDB、不再弹选择器。首次或授权丢失才弹一次文件夹选择。连点会合并成一次写入；坚果云暂占文件时自动重试一次。改收藏/尺寸/主题 350ms 后自动写。发布脚本从坚果云 `UsersMap.json` 反查根目录（`NUTSTORE_ROOT` 仍可覆盖）。
