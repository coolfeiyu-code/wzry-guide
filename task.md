...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.9**（近7日按棋手补搜，收入明先生山鬼流等冷门套）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 识别四类（用上场英雄名）：花木兰复生、三分整备倒转、大河开团、日落海整备。
...
- 讲解：木兰/三分/大河/日落海 + 往生图腾（大司命+米莱狄/蒙恬/项羽，且无虞姬）。只据官方卡面，不编山鬼数值。
