...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.33**（助手与浮窗同窗口切换，只留一个窗口）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 浮窗：**助手和浮窗在同一个窗口里切换**，任何时刻只有一个窗口。点「对局浮窗」把当前窗口收起成浮窗视图；浮窗里「回到助手」切回完整助手。**刻意不新开窗口** —— 用户双击打开的窗口浏览器不让脚本关掉，「关旧窗 + 开新窗」会出现两个窗口同时挂着的中间态。用户可见名是「助手」不是「图鉴」。关键按钮加重显示。弹窗 × 直接关层，不依赖 `history.back()`（file:// 常失效）。
- 尺寸：脚本开的窗口（`window.name` 是 wxqMain/wxqHud）点浮窗时会 `resizeTo` 缩成小窗贴屏幕右侧；双击打开的大窗口缩不了，浮窗改用居中窄卡片（`body.hud-wide`）呈现，不再拉满整屏。站位在上、装备/运营/出牌在下。手拉尺寸按分辨率分桶记 `wxq-hud-size`。「适配屏幕」只对可缩放窗口显示。
...
- 多台机器：打开时读云上 `王者助手.json.js` 与本机**取并集**（收藏合起来、最多 8 条），云端比合并结果旧就回写一次。绝不能用「本机覆盖云端」。
- 各电脑直接打开该 HTML：配置脚本在最前面，页面脚本跑之前已生效，**不用导入、不用点**。收藏/取消收藏、拖动浮窗、改主题会在 250ms 后静默写回。浏览器规定「写文件夹」必须先授权一次，所以第一次要点一下左下角「开启自动保存」并选一次「王者万象棋助手」文件夹；之后一直静默，按钮自动隐藏。发布脚本从坚果云 `UsersMap.json` 反查根目录（`NUTSTORE_ROOT` 仍可覆盖）。
- 浮窗：复制失败弹出大字阵容码（点一下全选再 Ctrl+C）。顶栏只留「回到助手/适配屏幕（仅可缩放窗口）/图鉴开」。手机（触屏/窄屏）走底部抽屉：站位置顶吸住，下滑看装备/运营/出牌。
