...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.39**（更新阵容：官方 53 套、7 日新卡 44 张）。官方阵容库 **v1.2.0**（53 套）。近7日数据阵容 **v1.1.0**。
...
- 2026-09-19 实测：原始 449 → 保留 47（丢 402）。2026-09-20 再同步：原始 450 → 保留 **53**（丢 397），同名不同阵容码的套（如「最强奕星偷牌河洛」「明先生泳池派对」）算不同套，key 唯一即可。
