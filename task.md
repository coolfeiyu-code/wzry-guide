...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.44**（棋手配色、图鉴跳转、真实适配数据）。官方阵容库 **v1.3.0**（298 套）。近7日数据阵容 **v1.2.0**（25 新卡 + 278 叠加）。
...
- **棋手配色**：19 位各一固定色（`wanxiangqi-jobs.js` 的 `LORD_COLORS`，按官方 `WXQ_PLAYERS` 顺序分配，无黑色、无重复），卡片与详情共用，深浅主题都不换色。`.lc` 色标渲染棋手名，卡片上取前 3 位。
