...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.32**（修多机配置同步：去掉 `<base>`、收藏取并集）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 浮窗：点「对局浮窗」用独立窗口打开同一份 `王者助手.html#hud-阵容码`（关掉原助手页小窗还在）。原页切到「浮窗已打开」占位。小窗「回到助手」只唤醒/新开完整大窗口，不把主页塞进小窗。用户可见名是「助手」不是「图鉴」。关键按钮加重显示。弹窗 × 直接关层，不依赖 `history.back()`（file:// 常失效）。
- 尺寸：默认弹出系统小窗（可拉到接近整屏高），站位在上、装备/运营/出牌在下，不再左右分栏。格子大小随屏幕高度。贴在屏幕右侧。手拉尺寸按分辨率分桶记 `wxq-hud-size`。顶栏「适配屏幕」清掉本分辨率手拉记录并重算。
...
- 浮窗：复制失败弹出大字阵容码（点一下全选再 Ctrl+C）。顶栏只留「回到助手/适配屏幕/图鉴开/关闭」。手机（触屏/窄屏）走底部抽屉：站位置顶吸住，下滑看装备/运营/出牌。
