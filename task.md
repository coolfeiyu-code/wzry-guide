...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.18**（坚果云配置打开即载入、保存即覆盖）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 尺寸：按当前屏幕 `availWidth×availHeight` 算最佳：格子大小随高度（720/1080/1440/4K），≥1440 宽用左右分栏（棋盘列+说明列），高度约 84–90%。贴在屏幕右侧。旧的全局 `{w,h}` 不再沿用；手拉过的尺寸按分辨率分桶记 `wxq-hud-size`。顶栏「适配屏幕」清掉本分辨率手拉记录并重算。字号随 `data-hud-fit=sm/md/lg/xl`。
...
- 各电脑直接打开该 HTML，会自动读同目录 `王者助手.json.js`，**不用导入**。点「保存配置」才写入/覆盖这份文件（后保存的覆盖先保存的）。不点保存，只留在这台浏览器。第一次保存要选坚果云根目录（和 html 放一起的那个）。
