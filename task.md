...
> 当前版本状态：站点 `GUIDE_META` **v2.6.12**；万象棋 `WXQ_META` **v1.5.12**（对局浮窗改为站位/装备合成/前中后期/出牌）。官方阵容库 **v1.2.0**（47 套）。近7日数据阵容 **v1.1.0**。
...
- 浮窗内容只放牌局里用得到的：7×4 站位（上=前排）、关键人物装备及官方 `craftFrom`（从什么合成）、前中后期运营原文（含回合和上阵名单）、出牌（讲解回合走法 + 从 brief/ops/effectDesc 抽出的手里/露娜/整备等句子）。不编胜率，原文没有的段就空着。游戏全屏独占会挡住网页，需窗口化。
